...
int SchedulerEntryPoint(void* arg)
{
  ...
  strcpy_s(devices[THREADS_CLOCK_DEVICE_ID].deviceName, sizeof(devices[THREADS_CLOCK_DEVICE_ID].deviceName), "clock");
 

    for (int i = 1; i < THREADS_MAX_DEVICES; ++i)
    {
        devices[i].deviceMbox = mailbox_create(5, sizeof(int));
    }
    int deviceHandle;
    deviceHandle = device_initialize("disk0");
    strcpy_s(devices[deviceHandle].deviceName, sizeof(devices[deviceHandle].deviceName), "disk0");
    deviceHandle = device_initialize("disk1");
    strcpy_s(devices[deviceHandle].deviceName, sizeof(devices[deviceHandle].deviceName), "disk1");
    deviceHandle = device_initialize("term0");
    strcpy_s(devices[deviceHandle].deviceName, sizeof(devices[deviceHandle].deviceName), "term0");
    deviceHandle = device_initialize("term1");
    strcpy_s(devices[deviceHandle].deviceName, sizeof(devices[deviceHandle].deviceName), "term1");
    deviceHandle = device_initialize("term2");
    strcpy_s(devices[deviceHandle].deviceName, sizeof(devices[deviceHandle].deviceName), "term2");
    deviceHandle = device_initialize("term3");
    strcpy_s(devices[deviceHandle].deviceName, sizeof(devices[deviceHandle].deviceName), "term3");  
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        devices[i].deviceHandle = (void*)(intptr_t)handle;
            strncpy_s(devices[i].deviceName, sizeof(devices[i].deviceName), deviceNames[i], _TRUNCATE);
...
static void io_handler(void* device, uint8_t command, uint32_t status, void* pArgs)
{
    ...
    unit.QuadPart = (LONGLONG)device; // We cast the pointer device into a 64-bit number
    deviceId = unit.LowPart;
    ...
    result = mailbox_send(devices[deviceId].deviceMbox, &status, sizeof(int), FALSE);
    //more code to check the return result
    if (result < 0) {
